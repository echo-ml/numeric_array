...
BENCHMARK_SET("strided_expression1", NumTrials(5), NumEpochs(1),
  ...
  NumericArray<double, Shape<index_t>> X1(make_shape(N)),
      Y1(make_shape(N_div_2));
  std::fill_n(X1.data(), N, 0);
  std::fill_n(Y1.data(), N_div_2, 0);
  auto sub_x1_eval = make_numeric_subarray_evaluator(X1.data(), make_strides(2_index));
  auto sub_x1_expr =
      make_numeric_array_expression<execution_context::structure::general>(
          make_dimensionality(N / 2), sub_x1_eval);
...
BENCHMARK_SET("strided_expression2", NumTrials(5), NumEpochs(1),
  ...
  auto sub_x1_eval = make_numeric_subarray_evaluator(X1.data(), strides);
  auto sub_y1_eval = make_numeric_subarray_evaluator(Y1.data(), strides);
  auto sub_x1_expr =
      make_numeric_array_expression<execution_context::structure::general>(
          make_dimensionality(N / 2), sub_x1_eval);
  auto sub_y1_expr =
      make_numeric_array_expression<execution_context::structure::general>(
          make_dimensionality(N / 2), sub_y1_eval);
