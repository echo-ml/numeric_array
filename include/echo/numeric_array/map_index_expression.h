...
auto map_index(const Functor& functor, Extents... extents) {
  return make_numeric_array_expression<execution_context::structure::general>(
      make_k_shape(extents...),
      make_map_index_evaluator<sizeof...(Extents)>(functor));
}
...
auto map_index(const Functor& functor, const Shape& shape) {
  return make_numeric_array_expression<execution_context::structure::general>(
      shape,
      make_map_index_evaluator<shape_traits::num_dimensions<Shape>()>(functor));
}
