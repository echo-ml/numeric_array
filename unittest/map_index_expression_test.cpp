...
TEST_CASE("map_index_expression") {
  ...
  auto expr3 = map_index<execution_context::structure::general>(f1, 3);

  auto expr4 = map_index(f1, make_k_shape(3));
  auto eval4 = expr1.evaluator();
  CHECK(eval4(0) == 0);
  CHECK(eval4(1) == 1);

  auto expr5 =
      map_index<execution_context::structure::general>(f2, make_k_shape(3, 2));
}
