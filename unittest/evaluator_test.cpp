...
TEST_CASE("numeric_subarray_evaluator2") {
  ...
  auto strides = make_strides(3_index, 1);
  auto eval1 = make_numeric_subarray_evaluator(&data[0], strides);
...
TEST_CASE("map_evaluator") {
  auto scalar_evaluator1 =
      make_flatten_evaluator<2>(make_scalar_evaluator(3.0));
  auto scalar_evaluator2 =
...
TEST_CASE("map_evaluator2") {
  ...
  auto eval1 = make_flatten_evaluator<2>(
      make_contiguous_numeric_array_evaluator(&data1[0]));
  auto eval2 = make_flatten_evaluator<2>(
      make_contiguous_numeric_array_evaluator(&data2[0]));
...
TEST_CASE("map_evaluator3") {
  ...
  std::iota(std::begin(data), std::end(data), 0);
  auto eval2 = make_flatten_evaluator<2>(
      make_contiguous_numeric_array_evaluator(&data[0]));
